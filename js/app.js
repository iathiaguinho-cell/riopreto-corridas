...
function fetchAllData() {
    ...
    db.ref('corridas').on('value', (snapshot) => {
        todasCorridas = snapshot.val() || {};
        renderCalendar(todasCorridas);
    });
    ...
    db.ref('resultadosEtapas').on('value', (snapshot) => {
        ...
        if (Object.keys(todasCorridas).length > 0) {
            renderCalendar(todasCorridas);
        }
    });
}

function renderCalendar(corridas) {
    ...
    corridasArray.forEach(corrida => {
        ...
        const hasResults = resultadosEtapas[corrida.id];
        const resultadosBtnHTML = hasResults ?
...
function searchAthlete(raceId) {
    ...
    output.innerHTML = `
        ...
        ${foundAthletes.map(atleta => `
            <div class="athlete-card-small">
                <div class="font-semibold text-white">${atleta.nome}</div>
                <div class="text-xs text-gray-400">#${atleta.classificacao} | ${atleta.tempo}</div>
            </div>
        `).join('')}
        </div>`;
}
